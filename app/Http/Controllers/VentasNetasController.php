...
class VentasNetasController extends Controller
{
    ...
    public function unit_sales(Request $request)
    {
        ...
        //fin del generador del arreglo
         ...
         foreach ($infoSales as $infoD) {
            $aceiteUnit = round($infoD->ACEITES, 3);
            $margaUnit = round($infoD->MARGARINAS, 3);
            $solCrUnit = round($infoD->SOLIDOS_CREMOSOS, 3);
            $totlPUnit = $aceiteUnit + $margaUnit + $solCrUnit;
            $indusUnit = round($infoD->INDUSTRIALES, 3);
            $aglAcid = round($infoD->ACIDOS_GRASOS_ACIDULADO, 3);
        ...
        //informacion de vista ventas toneladas
        foreach ($infoTons as $infoT) {
            $aceiUnit2 = round($infoT->TON_ACEITES, 3);
            $margaUnit2 = round($infoT->TON_MARGARINAS, 3);
            $solCrUnit2 = round($infoT->TON_SOLIDOS_CREMOSOS, 3);
            $totlPTUnit2 = $aceiUnit2 + $margaUnit2 + $solCrUnit2;
            $indusUnit2 = round($infoT->TON_INDUSTRIALES_OLEO, 3);
            $agAcid2 = round($infoT->TON_ACIDOS_GRASOS_ACIDULADO, 3);
            $servMaq2 = round($infoT->TON_SERVICIO_MAQUILA, 3);
            $totlVen2 = intval($totlPTUnit2) + intval($infoT->TON_INDUSTRIALES_OLEO) + intval(round($infoT->TON_ACIDOS_GRASOS_ACIDULADO));
            array_push($infoTs, [$aceiUnit2, $margaUnit2, $solCrUnit2, $totlPTUnit2, $indusUnit2, $agAcid2, $servMaq2, $totlVen2]);
        }
        //fin de informacion
